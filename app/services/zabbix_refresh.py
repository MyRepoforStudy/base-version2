...
def maybe_refresh_zabbix_cache(db: Session, force: bool = False) -> str | None:
    ...
    if not settings.zabbix_url or not settings.zabbix_api_token or settings.zabbix_auto_refresh_seconds <= 0:
        return None
    ...
    try:
        ...
        if created + updated + deleted == 0:
            _last_error = "Zabbix refresh returned 0 hosts for the configured group."
        else:
            _last_error = None
